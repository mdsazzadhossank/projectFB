...
export const MessagingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeConversationId, setActiveConversationId] = useState<string | null>('conv_01');
  const [messages, setMessages] = useState<Record<string, Message[]>>(INITIAL_MESSAGES);
  const [contacts, setContacts] = useState<Contact[]>(INITIAL_CONTACTS);
  const [integrations, setIntegrations] = useState<IntegrationChannel[]>(INITIAL_INTEGRATIONS);
  ...
  const sendMessage = (conversationId: string, text: string, attachment?: Attachment) => {
    ...
    // Simulate realistic customer auto-reply after 3.5s for demo delight
    setTimeout(() => {
      const replyTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      const automatedReplies = [
        'Thank you for the quick update! That sounds great.',
        'Noted! I will review the details and get back to you shortly.',
        'Understood. Thanks for assisting so quickly on this!',
        'Perfect, looking forward to starting the process.',
      ];
      ...
      const incomingMsg: Message = {
        ...
        conversationId,
        sender: 'customer',
        senderName: activeConversation?.customerName || 'Customer',
        text: randomReply,
        timestamp: replyTime,
        status: 'read',
      };
      ...
      setMessages((prev) => {
        ...
        return {
          ...prev,
          [conversationId]: [...currentList, incomingMsg],
        };
      });
